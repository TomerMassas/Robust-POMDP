...
New vs. the src/robust_pomdp tree:
  ...
  - ActionNode carries S_next (union of its children's S_in = the sampled
    next-state support) and a per-state survival scalar `c`.
  - HistoryNode carries a per-state survival scalar `c` (greedy-policy value).

`c` and beliefs are keyed by state index (dict) so cross-node lookups
(child.c.get(s', 0.0)) stay clean regardless of differing supports.
...
class ActionNode:
    ...
    Attributes:
        ...
        c: dict state -> survival scalar (leakage certificate), over the parent's S_in.
    """
...
class HistoryNode:
    ...
    Attributes:
        ...
        c: dict state -> survival scalar under the greedy policy.
    """
