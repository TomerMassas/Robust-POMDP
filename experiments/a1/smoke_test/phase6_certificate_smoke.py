...
from experiments.a1.synthetic_pomdp import (ABORT, ACTION_NAMES, PROCEED,
                                             make_initial_belief, make_synthetic_pomdp)
from experiments.a1.tree_evaluator import build_policy_tree_by_path, evaluate_robust_value
...
def _build(model, b0, H, S_in, Z_in):
    return build_policy_tree_by_path(
        model, _const_policy, b0, H, S_in=S_in, Z_in=Z_in,
        abort_action=ABORT,
    )
...
def main() -> None:
    ...
    full_html = out_dir / "phase6_cert_tree_full.html"
    proj_html = out_dir / "phase6_cert_tree_projected.html"
    render_tree_to_html(root_full, nd_full, ACTION_NAMES, full_html,title=f"Phase 6 cert - full tree (N={N}, M={M}, H={H})")
    render_tree_to_html(root_proj, nd_proj, ACTION_NAMES, proj_html,title=f"Phase 6 cert - projected tree "f"(|S_in|={len(S_in_proj)}, |Z_in|={len(Z_in_proj)})")
    print(f"Wrote tree viz: {full_html.name}, {proj_html.name}\n")

    # ---- Test 1: rho=0, full support: certificate = 0 ----
    unc_zero = uniform_uncertainty(N, A, 0.0, 0.0, TVDistance())
    V_full_z = evaluate_robust_value(root_full, nd_full, model, unc_zero)
    cert_fullsup = compute_certificate(root_full, nd_full, model, unc_zero, b0, H)
    assert abs(cert_fullsup["certificate"]) < 1e-9, (f"full + rho=0: certificate = {cert_fullsup['certificate']}, expected 0")
    print(f"  rho=0, full support: V_full={V_full_z:.4f}  cert=0  OK")


    # ---- Test 2: rho=0, partial support: certificate >= true_error ----
    V_proj_z = evaluate_robust_value(root_proj, nd_proj, model, unc_zero)
    cert_p_z = compute_certificate(root_proj, nd_proj, model, unc_zero, b0, H)
    err_p_z = abs(V_full_z - V_proj_z)
    assert err_p_z <= cert_p_z["certificate"] + 1e-6, (f"partial + rho=0: true_error {err_p_z} > cert {cert_p_z['certificate']}")
    ...
    print(f"        delta_b={cert_p_z['delta_b']:.4f}  R_max={cert_p_z['R_max']:.1f}  m_in={cert_p_z['m_in']:.4f}")
    ...
    cert_p_r = compute_certificate(root_proj, nd_proj, model, unc_mid, b0, H)
    err_p_r = abs(V_full_r - V_proj_r)
    assert err_p_r <= cert_p_r["certificate"] + 1e-6, (f"partial + rho={rho}: true_error {err_p_r} > cert {cert_p_r['certificate']}")
