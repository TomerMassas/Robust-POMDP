"""
Synthetic ordered-risk POMDP for A1 (Fixed-Policy Projected Robust Tree Evaluator).

Hidden state space S = {0, ..., N-1} ordered by hidden risk:
  - Low-index states  = safe
  - Middle-index      = risky
  - High-index states = catastrophic

Action space A = {INSPECT, PROCEED, MITIGATE, ABORT}.
Observation space Z = {0, ..., M-1} ordered by warning severity.

Defaults match the A1 design PDF (`pdfs/planner A - design.pdf`).
"""
...
def class_sizes(N: int,
    ...
    """Partition N into (n_safe, n_risky, n_cat).

    Catastrophic is clamped to >= 1 so small debugging domains still have a cat state.
    """
...
def make_synthetic_pomdp(N: int,
    ...
    Transitions are translation-invariant on interior states (boundary clipping
    at s=0 for mitigate and s=N-1 for inspect/proceed). Abort self-loops to keep
    T well-defined; terminal handling lives in the tree builder.

    Transition: (N, N_ACTIONS, N) --> T[s, a, s_next] = P_T^o(s_next | s, a)
    Observation: (N, M) --> O[s_next, z] = P_Z^o(z | s_next)
    Reward: (N, N_ACTIONS) --> R[s, a]
    """
    if N < 2:
